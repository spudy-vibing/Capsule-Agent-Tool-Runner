...
| 4 | **Milestone Order** | Linear sequence (original) | Simpler to track: alpha.1 → alpha.2 → alpha.3 → beta.1 → release |

### Completed Planning Topics

- [x] Built-in pack detailed design (Local Doc Auditor, Repo Analyst) ✅ See Appendix C
- [x] Prompt engineering strategy for JSON output ✅ See Appendix D
- [x] Threat model review for planner integration ✅ See Appendix E
- [x] Documentation plan (README updates, guides) ✅ See Appendix F

**All planning topics complete. Ready to begin implementation with v0.1.1 → alpha.1**

### Key Architectural Decisions

**Planner Integration:**
```
Capsule ──HTTP──▶ Ollama (external)
                  └── User manages model pulls
```
...
v0.3: ~/.capsule/packs/ (user-installed)
      └── UserPackSource + capsule pack install
```
...
```
┌─────────────────────┐  ┌─────────────────────┐
│ Deterministic Evals │  │ Planner Evals       │
│ (CI-safe)           │  │ (local only)        │
│ • Policy tests      │  │ • Task completion   │
│ • YAML execution    │  │ • Compliance rate   │
└─────────────────────┘  └─────────────────────┘
```
...
|-----------|--------|-------|
| **CLI Commands** | ✅ Complete | run, replay, report, list-runs, show-run |
| **Tools** | ✅ Complete | fs.read, fs.write, http.get, shell.run |
| **Policy Engine** | ✅ Complete | deny-by-default, path/domain/token/quota rules |
| **Storage** | ✅ Complete | SQLite with plan_hash, policy_hash, full audit |
| **Replay Engine** | ✅ Complete | Deterministic replay with hash verification |
| **Reporting** | ✅ Complete | Console (Rich) + JSON export |
| **Error Handling** | ✅ Complete | Numeric codes, context dicts, actionable suggestions |
| **CI/CD** | ✅ Complete | Python 3.11/3.12/3.13, Ubuntu + macOS, Codecov |
| **Tests** | ✅ 397 tests | Unit, integration, security categories |
| **Documentation** | ✅ Good | README, architecture.md, examples |
...
| **Agent Loop** | P0 | SLM proposes → policy enforces → return result |
...
- **FR-04.1:** Pack directory structure:
  ```
  packs/<pack_name>/
  ├── manifest.yaml       # Pack metadata, dependencies, entry point
  ├── policy.yaml         # Default policy for this pack
  ├── prompts/
  │   └── system.txt      # System prompt template
  ├── plans/
  │   └── default.yaml    # Example YAML plan (for non-SLM mode)
  └── evals/
...
|----------|----------|---------------|---------|
| **Deterministic** | ✅ Yes | No | Policy tests, YAML execution |
| **Planner** | ❌ No (local only) | Yes (Ollama) | Task completion, compliance |
...
│                    Execution Layer                       │
│   ┌─────────────┐  ┌──────────────┐  ┌──────────────┐   │
│   │ PolicyEngine│  │ ToolRegistry │  │   Reporter   │   │
│   │ (evaluate)  │  │ (fs,http,sh) │  │ (json,term)  │   │
│   └─────────────┘  └──────────────┘  └──────────────┘   │
└─────────────────────────┬───────────────────────────────┘
                          │
┌─────────────────────────▼───────────────────────────────┐
│                    Storage Layer                         │
│                  SQLite (runs, calls, results)          │
└─────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│                        CLI Layer                         │
│   (run, replay, report, list-runs, show-run)            │
│   NEW: pack list/info/run/validate, agent run           │
└─────────────────────────┬───────────────────────────────┘
                          │
┌─────────────────────────▼───────────────────────────────┐
│                   Orchestration Layer                    │
│  ┌──────────┐ ┌─────────────┐ ┌───────────────────────┐ │
│  │  Engine  │ │ ReplayEngine│ │     AgentLoop         │ │
│  │ (YAML)   │ │  (replay)   │ │ (planner-driven)  NEW │ │
│  └──────────┘ └─────────────┘ └───────────────────────┘ │
└─────────────────────────┬───────────────────────────────┘
                          │
                          │  ┌────────────────────────────┐
...
| **AgentLoop** | Orchestrate planner ↔ policy ↔ tool cycle | **NEW** |
...
└─────┬─────┘     JSON tool call         └─────────────┘
      ...
      ▼
┌───────────┐
│  Policy   │  4. evaluate(call, policy)
│  Engine   │
└─────┬─────┘
      │
      │  5a. If Denied → return PolicyDeniedError to planner
      │  5b. If Allowed → continue
      ▼
┌───────────┐
│   Tool    │  6. execute(call)
│  Execute  │
└─────┬─────┘
      │
      │  7. ToolResult (success/error)
      ▼
┌───────────┐
│  Store    │  8. Log call, result, proposal
│  (SQLite) │
└─────┬─────┘
      │
      │  9. Return result to planner (loop back to step 2)
...
│   ├── __init__.py
│   ├── __main__.py
│   ├── cli.py                    # Extended with pack/agent commands
│   ├── engine.py                 # No change
│   ├── errors.py                 # Extended with PlannerError
│   ├── schema.py                 # Extended with Planner schemas
│   │
│   ├── policy/
│   │   └── engine.py             # No change
│   │
...
│   │
│   ├── store/
│   │   └── db.py                 # Extended with planner_proposals
│   │
│   ├── planner/                  # NEW: Planner subsystem
│   │   ├── __init__.py
│   │   ├── base.py               # Planner ABC, PlannerState, Done
│   │   ├── ollama.py             # OllamaPlanner adapter
│   │   ├── prompt.py             # Prompt builder utilities
│   │   └── json_repair.py        # JSON parsing/repair utilities
│   │
│   ├── agent/                    # NEW: Agent loop orchestration
│   │   ├── __init__.py
│   │   └── loop.py               # AgentLoop class
│   │
│   └── pack/                     # NEW: Pack system
│       ├── __init__.py
│       ├── loader.py             # PackLoader, manifest parsing
│       ├── manifest.py           # Pack manifest schema
│       └── eval.py               # EvalHarness
│
├── packs/                        # NEW: Built-in packs
│   ├── local_doc_auditor/
│   │   ├── manifest.yaml
│   │   ├── policy.yaml
│   │   ├── prompts/
...
│   ├── unit/
│   │   ├── ...existing...
│   │   ├── test_planner_base.py      # NEW
│   │   ├── test_planner_ollama.py    # NEW
│   │   ├── test_agent_loop.py        # NEW
│   │   ├── test_pack_loader.py       # NEW
│   │   └── test_pack_eval.py         # NEW
│   │
│   ├── integration/
│   │   ├── ...existing...
│   │   ├── test_agent_integration.py # NEW
│   │   └── test_pack_integration.py  # NEW
│   │
│   └── security/
│       └── ...existing...
│
├── examples/
│   ├── ...existing...
│   ├── agent_demo.yaml           # NEW: Agent mode demo
│   └── pack_demo/                # NEW: Custom pack example
│
├── docs/
│   ├── architecture.md           # Updated
│   ├── threat_model.md           # Existing (ensure complete)
│   ├── planner_guide.md          # NEW: Planner integration guide
│   ├── pack_authoring.md         # NEW: How to create packs
│   └── planning/
│       └── v0.2-build-plan.md    # This document
│
...
|---------|--------|-------------|
| v0.0 | ✅ Complete | Project scaffolding, CI setup |
| v0.1.0 | ✅ Complete | Core tools, policy, replay, reporting |
| v0.2.0 | 🔄 Planned | SLM planner, packs, agent loop |
...
1. **Agent Loop Core (P0)**
   ...
   - [ ] Loop: propose → evaluate → execute → return
...
### 5.5 v0.2.0-beta.1 - Eval Harness

**Goal:** Implement evaluation framework for testing packs and planners.
...
tests/
├── unit/
│   ├── test_planner_base.py        # Planner interface tests
│   ├── test_planner_ollama.py      # OllamaPlanner with mocks
│   ├── test_planner_json_repair.py # JSON repair utilities
│   ├── test_agent_loop.py          # AgentLoop unit tests
│   ├── test_pack_manifest.py       # Pack manifest validation
│   ├── test_pack_loader.py         # Pack loading tests
│   └── test_pack_eval.py           # Eval harness tests
│
├── integration/
│   ├── test_agent_integration.py   # Full agent loop with mocks
│   ├── test_pack_yaml_mode.py      # Pack execution in YAML mode
│   └── test_pack_agent_mode.py     # Pack execution with mocked SLM
│
└── fixtures/
    ├── mock_ollama.py              # Ollama mocking utilities
    └── sample_packs/               # Test pack fixtures
```
...
1. **Model Management**
   ...
   - ❌ Quantization
   - ❌ Model registry
   - ❌ Model serving (use Ollama/llama.cpp externally)
   - ❌ Fine-tuning
...
2. **Advanced Agent Features**
   ...
   - ❌ Memory/RAG integration (beyond conversation history)
...
5. **Advanced Execution**
   - ❌ Parallel/async step execution
   - ❌ DAG-based plans
   - ❌ Conditional execution
   - ❌ Rollback/transactions

6. **Additional Tools**
   - ❌ fs.delete, fs.list (potential security concerns)
   - ❌ http.post/put/delete (scope creep)
   - ❌ Database tools
   - ❌ Email tools
...
├── policy.yaml
├── prompts/
│   ├── system.txt          # SLM mode system prompt
│   └── patterns.txt        # Reference patterns (included in prompt)
├── plans/
│   └── default.yaml        # YAML-only mode execution plan
├── evals/
│   └── test_cases.yaml
└── patterns/
    └── secrets.yaml        # Regex patterns for detection
```
...
1. **Minimal Tool Surface:** Each pack should need ≤2 tools
...
│   <response>{"tool": "fs.read", "args": {"path": ...}}</response>"│
└─────────────────────────────────┬───────────────────────────┘
                                  │
                    ┌─────────────▼─────────────┐
                    │  1. Extract from tags     │
                    ...
                    └─────────────┬────────────┘
                                  │
                         Max retries? ─ Yes ─→ PlannerParseError
                                  │
                                  └──→ Back to SLM
```
...
| 2 | Input sanitization - Strip known injection patterns | ⚠️ Limited effectiveness |
| 3 | Structured prompts - XML tags separate instructions from user input | ✅ Designed (Appendix D) |
...
| # | Mitigation | Status |
|---|------------|--------|
| 1 | Path canonicalization - Resolve `..` before policy check | ✅ Implemented (v0.1) |
| 2 | Allowlist approach - Only explicitly allowed paths pass | ✅ Implemented |
| 3 | Symbolic link resolution - Prevent symlink escapes | 🔲 **TODO v0.2** |
| 4 | Unit tests - Extensive path traversal tests exist | ✅ Implemented |
...
| 2 | Private IP blocking - Block RFC1918, link-local, localhost | 🔲 **TODO v0.2** |
| 3 | No redirects to different hosts - Re-evaluate policy on redirect | ✅ Implemented |
| 4 | DNS rebinding protection - Resolve DNS before request, verify IP | 🔲 TODO v0.3 |
...
| # | Mitigation | Status |
|---|------------|--------|
| 1 | Command allowlist - Only pre-approved commands | ✅ Implemented |
...
| 4 | Sandbox execution - bubblewrap/firejail isolation | 🔲 TODO v0.3 |
...
| 4 | Total execution timeout - Overall time limit | 🔲 **TODO v0.2** |
...
```
Step 1: Read /home/user/.env → Contains API_KEY=sk-secret123
Step 2: SLM now has API_KEY in context
Step 3: User asks "what did you find?" → SLM may output the key
```
...
| 3 | Permission manifest - User approves pack permissions | 🔲 TODO v0.3 |
| 4 | Sandboxed pack execution - Packs can't escape their policy | 🔲 TODO v0.3 |

**Status:** ✅ Mitigated in v0.2 by not supporting external pack installation.
...
| 4 | Documentation - Warn users about log contents | 🔲 **TODO v0.2** |
...
| 3 | Authentication - Support Ollama auth tokens | 🔲 TODO v0.3 |

**Status:** ✅ Acceptable for v0.2 (localhost only).
...
## Overview
├── What is the planner?
├── YAML mode vs Agent mode comparison
└── When to use each
...
│   ├── qwen2.5:0.5b - Fastest, lower quality
│   ├── qwen2.5:1.5b - Balanced (recommended)
│   └── llama3.2:3b - Best quality, slower
└── Using `capsule doctor`
...
## How It Works
├── Agent loop explained (with diagram)
├── Policy enforcement
├── JSON output format
└── Done signal

## Troubleshooting
├── "Ollama not found" - run `capsule doctor`
├── JSON parse errors - model may need upgrade
├── Timeout issues - increase timeout or use smaller model
└── Policy denials - review policy.yaml

## Advanced
├── Verbose output for debugging
├── Custom system prompts
└── Future: Custom planner backends
```
...
├── policy.yaml        # Required: Default policy
├── prompts/
│   └── system.txt     # Optional: SLM system prompt
├── plans/
│   └── default.yaml   # Optional: YAML mode plan
└── evals/
    └── test_cases.yaml # Optional: Test cases

## Manifest Reference
├── Required fields (name, version, description, tools_required)
├── Input schema definition
├── Output schema definition
└── Metadata (author, license, capsule_version)

## Writing Policies
├── Restrictive by default
├── Path patterns with globs
├── Domain allowlists for http.get
└── Testing policy with edge cases
...
├── store_as for results
└── continue_on_error

## Testing Packs
├── `capsule pack validate <path>`
├── Writing eval test cases
├── Manual testing workflow
└── CI integration

## Best Practices
├── Security: minimal tool surface
...
## Overview
├── Capsule's security model
├── Trust boundaries diagram
└── Deny-by-default principle

## Audit Logs

⚠️ IMPORTANT: Capsule logs ALL tool results to SQLite.

### What Gets Logged
├── Tool call arguments
├── Tool results (including file contents!)
├── Planner proposals
└── Policy decisions

### Protecting Sensitive Data
├── Use encrypted volumes for DB location
├── Custom DB path: --db /secure/path/audit.db
├── Review logs before sharing
└── Delete old runs: rm ~/.capsule/runs.db

## Policy Configuration

### Path Security
├── Always use absolute paths in policy
├── Symlinks resolved before policy check
...
├── Avoid wildcards like *.com
└── Consider egress restrictions

### Shell Security
├── Command allowlist approach
├── No shell=True (no interpolation)
├── Validate argument patterns
└── Consider sandbox for sensitive ops

## Planner Security

### Untrusted Output
├── SLM output is NEVER trusted
├── Policy evaluates EVERY call
├── Invalid JSON = retry, not execute
└── Bounded retries prevent loops

### Prompt Injection
├── Users may try injection attacks
├── Policy is the security boundary
├── SLM may be tricked, policy isn't
...
5. If denied → return denial to planner
6. If allowed → execute tool
